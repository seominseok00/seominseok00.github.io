...
- Centralized value function만 가지고는 decentralized 방식으로 효율적으로 행동을 선택하기 어렵다.
  - 만약 centralized information이 이용가능하더라도 centralized action-value function으로 greedy actions을 선태하는 것은 방대한 joint-action space 때문에 계산 비용이 많이 들게 된다.
...
하지만 이러한 방식은 간단한 문제에서도 실패하는데, 하나의 에이전트만 활성화되고 나머지 에이전트들은 게으른 상태로 남는 lazy agent 문제가 발생한다. (첫 번째 에이전트각 유용한 정책을 학습했을 때, 두 번째 에이전트의 탐색이 첫 번째 에이전트를 방해해 team reward를 약화시킬 수 있는데, 이때문에 학습을 꺼리게 될 때 발생하는 현상이다)
...
또한 부분적으로만 관측 가능하기 때문에(partially observable), 다른 에이전트의 행동에서 발생하는 보상 신호를 자신의 것으로 오해할 수도 있다. (spurious rewards problem)
...
> 이에 대한 증명은 [Multi-Agent Reinforcement Learning: Foundations and Modern Approaches](https://www.marl-book.com/) 책의 9.5.2절에 나오는데, 기회가 된다면 나중에 따로 정리하도록 하겠다.
